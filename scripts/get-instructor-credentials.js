...
async function getInstructorCredentials() {
  try {
    ...
    for (const instructor of instructors) {
      ...
      if (!user) {
        console.log(`⚠️  Instructor ${instructor.email} has no associated user record.\n`);
        continue;
      }
      ...
      console.log(`🏷️  RFID Tag:     ${instructor.rfidTag}`);
      ...
      if (!passwordFound) {
        console.log(`   ⚠️  Password: Unknown (not matching common defaults)`);
